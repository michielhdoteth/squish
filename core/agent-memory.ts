...
export async function searchAgentMemories(
  ...
  try {
    ...
    where = and(where, inArray(schema.memories.id,
      await (db as any)
        .select({ id: schema.memories.id })
        .from(schema.memories)
        .where((schema.memories.content as any).ilike(`%${query}%`))
    ));
 ...
 */
export async function getPinnedMemories(context: AgentContext, limit: number = 10): Promise<any[]> {
  try {
    const db = await getDb();
    const schema = await getSchema();
    ...
    const memories = await (db as any)
      ...
      .where(
        and(
          eq(schema.memories.agentId, context.agentId),
          eq(schema.memories.isPinned, true)
        )
      )
      .limit(limit);
 ...
 */
export async function canAgentAccessMemory(
  memoryId: string,
  context: AgentContext
): Promise<boolean> {
  try {
    ...
    if (m.agentId === context.agentId) return true;

    // Check visibility scope
    if (m.visibilityScope === 'private') return false;

    if (m.visibilityScope === 'project' && m.projectId !== context.projectId) return false;

    if (m.visibilityScope === 'team') return true;

    if (m.visibilityScope === 'global') return true;
...
export async function listAgents(): Promise<any[]> {
  try {
    ...
    const agents = await (db as any)
      .selectDistinct({
        agentId: schema.memories.agentId,
        agentRole: schema.memories.agentRole,
      })
      .from(schema.memories)
      .where((schema.memories.agentId as any).isNotNull());
...
export async function getAgentStats(context: AgentContext): Promise<{
  ...
  try {
    ...
    for (const mem of memories) {
      ...
      if (mem.visibilityScope !== 'private') stats.sharedMemories++;
    }
