...
node.on('mouseover',(e,d)=>{tip.style('display','block').html('<b>'+d.label+'</b><br/>'+d.type+(d.group?' &middot; '+d.group:'')).style('left',(e.pageX+10)+'px').style('top',(e.pageY+10)+'px');d3.select(e.currentTarget).attr('stroke-width',2)}).on('mousemove',e=>{tip.style('left',(e.pageX+10)+'px').style('top',(e.pageY+10)+'px')}).on('mouseout',e=>{tip.style('display','none');d3.select(e.currentTarget).attr('stroke-width',0.5)});
