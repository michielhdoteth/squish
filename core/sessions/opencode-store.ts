 ...
 * (`opencode.db`) so that `squish sessions list|search|show|related|inject`
 * can return PAST OpenCode sessions - not just manually-captured ones.
 *
 * This module is the bridge between the existing chunk-based Squish
 * memory store (in `store.ts`) and OpenCode's local SQLite store. The
 * chunk shape is identical: we synthesize Chunk objects from OpenCode
 * session + message + part rows, and return them via the same public
 * surface (`searchChunks`, `listSessionGroups`, etc.) when the caller
 * opts in via `--source=opencode` or `--source=all`.
 *
 ...
 *     content. `--source=opencode-deep` widens to all parts.
 ...
 *   - core/sessions/store.ts (the squish-memory store layers over this)
 *   - packages/cli/src/commands/sessions.ts (via --source=opencode|all)
 */
