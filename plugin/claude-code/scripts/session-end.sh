#!/bin/bash
# SessionEnd hook - captures the real end of a Claude Code session.
# Receives JSON on stdin: { session_id, transcript_path, cwd, reason, ... }
#
# Extracts the last user message and the final assistant text from the
# session transcript (JSONL) and stores a 2KB excerpt via `squish remember`.
# If no real content can be extracted, nothing is stored (no placeholders).
...
if [ -n "$EXCERPT" ]; then
  squish remember "$EXCERPT" --type context --place archive --project "$PROJECT_DIR" >/dev/null 2>&1
fi
