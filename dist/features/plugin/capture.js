...
export async function captureToolUse(projectPath, toolName, toolArgs, toolResult, context) {
    ...
    const observation = await captureObservation(projectPath, 'tool_use', toolName, projectPath, summary, { toolName, arguments: toolArgs, result: toolResult, tags: ['auto-captured', 'tool-use', toolName] }, context);
    if (observation) {
        try {
